...
import {
    Loader2, ChevronRight, Check, Building2, MapPin, Package, Wrench,
...
export function Step2DeliveryLinking() {
    ...
    // Applied values (debounced for text fields, immediate for toggles)
    ...
    // Fetch whenever the user is on the physical view and search/filters/page change.
    // loadAgencies has no deps (useCallback([])) so it never changes — the effect
    // only re-runs when productType, applied search/filter values, or page change.
    useEffect(() => {
        if (productType !== 'physical') return;
        loadAgencies(appliedFilters, appliedSearch, page);
    }, [productType, appliedFilters, appliedSearch, page, loadAgencies]);
    ...
    return (
        <OnboardingLayout ctaSlot={ctaSlot} stepKey={2}>
            ...
            {productType === 'physical' && (
                <div className="space-y-3">
                    ...
                    <Collapsible open={filtersOpen} onOpenChange={setFiltersOpen}>
                        ...
                        {/* Filter panel — full width, animated by Collapsible */}
                        <CollapsibleContent>
                            <div className="mt-3">
                                <FiltersPanel
                                    filters={filters}
                    ...
                    {fetchError ? (
                        ...
                        <ScrollArea className="h-[42vh] min-h-[160px]">
                            <div
                            ...
                            >
                                {agencies.map((agency) => (
                                    <AgencyCard
                                        ...
                                        onSelect={() => setSelectedId(agency.id)}
            ...
            <AgencyDetailSheet
                ...
                onSelect={() => {
                    if (detailAgency) setSelectedId(detailAgency.id);
                }}
            />
        </OnboardingLayout>
    );
