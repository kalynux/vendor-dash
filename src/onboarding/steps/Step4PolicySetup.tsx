...
export function Step4PolicySetup() {
    ...
    const [enableReturn, setEnableReturn] = useState(
        !!(draft?.return_policy ?? existingPolicies?.return_policy),
    );
    const [enableCancellation, setEnableCancellation] = useState(
        !!(draft?.cancellation_policy ?? existingPolicies?.cancellation_policy),
    );
    const [enableSupport, setEnableSupport] = useState(
        !!(draft?.support_policy ?? existingPolicies?.support_policy),
    );
    ...
    const {
        ...
        defaultValues: {
            ...
            cancellation_policy: draft?.cancellation_policy ?? (existingPolicies?.cancellation_policy
                ? {
            ...
            support_policy: draft?.support_policy ?? (existingPolicies?.support_policy
                ? {
    ...
    const handleSave = useCallback(
        async (values: Step4FormValues) => {
            setApiError(null);
            saveDraft(4, values);
            try {
                await submitPolicySetup({
                    skip: false,
                    ...(enableReturn && values.return_policy ? { return_policy: values.return_policy } : {}),
                    ...(enableCancellation && values.cancellation_policy ? { cancellation_policy: values.cancellation_policy } : {}),
                    ...(enableSupport && values.support_policy ? { support_policy: values.support_policy } : {}),
        ...
        [submitPolicySetup, saveDraft, enableReturn, enableCancellation, enableSupport, roleEntity?.version],
    );
    ...
    return (
        <OnboardingLayout ctaSlot={ctaSlot} stepKey={4}>
            ...
            <form id="step4-form" onSubmit={handleSubmit(handleSave)} className="space-y-4" noValidate>
