...
**Verified against source on 2026-09-08** — the registry census re-derived, and
**`vendor_booking_created` corrected from 3 body params to 5** (§ 3), against
`src/modules/whatsapp/handlers/template/template-registry.ts` (**65** registrations),
`src/modules/whatsapp/handlers/template/template-validator.ts:44`,
`src/modules/whatsapp/constants/whatsapp-limits.ts`, `src/core/constants/languages.ts` and the
four notification catalogs under `src/modules/notifications/catalog/` (**94** distinct template
names referenced).
*(First written against source 2026-08-24, when the registry held 61 and the catalogs 90.)*
...
>
> Filed as **F-31**. Re-derived 2026-09-08: the registry holds **65** names
> (`template-registry.ts`, one `[name, paramCount, hasButton, description]` row each).
> The four notification catalogs reference **94** distinct names. **Thirty are referenced and
> never registered**, and **eight of those are vendor templates that this page does not document
> at all** — see [§ 13](#13--eight-vendor-templates-missing-from-this-page).
>
> *(Both totals grew by four since 2026-08-24 — this page said 61 and 90. The gap itself is
> unchanged at thirty, and the same thirty names.)*
>
