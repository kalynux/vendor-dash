...
import {
  MAX_ATTACHMENTS, formatFileSize, ROLE_LABELS,
...
export function AttachmentsPanel({
  ...
  useEffect(() => {
    ...
    fetchAttachments(ticketId)
      ...
      .catch((err) => {
        if (active) toast.error(err instanceof ApiError ? err.message : 'Failed to load attachments');
      })
  ...
  async function handleSelect(files: ApiFile[]) {
    ...
    try {
      ...
      if (err instanceof ApiError) {
        if (err.code === 'TICKET_ATTACHMENT_LIMIT_EXCEEDED') {
          toast.error(`Maximum of ${MAX_ATTACHMENTS} attachments per ticket reached.`);
        } else if (err.code === 'TICKET_ACCESS_DENIED') {
          toast.error('You can only attach files you own.');
        } else {
      ...
      {loading ? (
        ...
        <ul className="space-y-2">
          {attachments.map((att) => {
            ...
            const uploader = att.uploadedByActor?.name ?? ROLE_LABELS[att.uploadedByRole];
            return (
      ...
      {!loading && !readOnly && !atLimit && (
        <div className="space-y-2">
          ...
          >
            ...
            {uploading ? 'Attaching…' : 'Upload attachment'}
          </button>
        </div>
      )}
